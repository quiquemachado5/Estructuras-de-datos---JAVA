import java.util.*;


public class PruebaAlumnos {
    public static void main(String[] args) {
        
        Alumno alumno1 = new Alumno("Enrique","29502554V");
        Alumno alumno2 = new Alumno("Pepe","29502054V");
        Alumno alumno3 = new Alumno("Amalia","12345678P");
         
        Asignatura asignatura1 = new Asignatura("Matemáticas",2010);
        Asignatura asignatura2 = new Asignatura("Lengua",2007);
        Asignatura asignatura3 = new Asignatura("Física",2019);
        
        
        Alumnos grupoAlumnos = new Alumnos();
        
        grupoAlumnos.agregarAlumnos(alumno1);
        grupoAlumnos.agregarAlumnos(alumno2);
        grupoAlumnos.agregarAlumnos(alumno3);
        
        
        alumno1.agregarAsignaturasMatriculadas(asignatura1);
        alumno1.agregarAsignaturasMatriculadas(asignatura2);
        
        alumno1.agregarAsignaturasAprobadas(asignatura1);
        
       
        alumno2.agregarAsignaturasMatriculadas(asignatura2);
        alumno2.agregarAsignaturasMatriculadas(asignatura3);
        
        alumno2.agregarAsignaturasAprobadas(asignatura2);
        alumno2.agregarAsignaturasAprobadas(asignatura3);
        
        
        alumno3.agregarAsignaturasMatriculadas(asignatura1);
        alumno3.agregarAsignaturasMatriculadas(asignatura3);
        
        alumno3.agregarAsignaturasAprobadas(asignatura3);
        alumno3.agregarAsignaturasAprobadas(asignatura1);
        
        
       System.out.println("Asignaturas matriculadas por alumno1:");
       Iterator ut = alumno1.getConjuntoAsignaturasMatriculadas().iterator();
       while(ut.hasNext()){
           System.out.println(" "+ut.next());
       }
       
       System.out.println("Matriculados en asignatura 1");
       Iterator it = grupoAlumnos.getMatriculados(asignatura1).iterator();
       while(it.hasNext()){
           System.out.println(" "+it.next());
       }
       
        System.out.println("No matriculados en asignatura 2: ");
       Iterator ot = grupoAlumnos.getNoMatriculados(asignatura2).iterator();
       while(ot.hasNext()){
           System.out.println(" "+ot.next());
       }
 
        System.out.println("Aprobados asignatura 1: ");
        Iterator et = grupoAlumnos.getAprobados(asignatura1).iterator();
        while(et.hasNext()){
            System.out.println(" "+et.next());
        }
        
        System.out.println("Ninguna asignatura aprobada: ");
        Iterator at = grupoAlumnos.getNingunaAprobada().iterator();
        while(at.hasNext()){
            System.out.println(" "+at.next());
        }
        
        System.out.println("Asignaturas matriculadas y aprobadas: ");
        Iterator pt = grupoAlumnos.getMatriculadosYAprobados(asignatura2,asignatura2).iterator();
        while(pt.hasNext()){
            System.out.println(" "+pt.next());
        }
    }
}
